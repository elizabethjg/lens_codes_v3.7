    ...
    def __init__(self,R,D_Sigma,err,z, cosmo,fitc = False):
        ...
        if fitc:
            ...
            try:
                ...
                NFW_out = curve_fit(NFW_profile,R,D_Sigma,sigma=err,absolute_sigma=True, bounds=([0.,0],[2.5,10]))
                pcov    = NFW_out[1]
...
class Sigma_fit:
    ...
    def __init__(self,R,Sigma,err,z, cosmo,fitc = False):
        ...
        if fitc:
            ...
            try:
                NFW_out = curve_fit(NFW_profile,R,Sigma,sigma=err,absolute_sigma=True,bounds=([0.,0],[2.5,10]))
                pcov    = NFW_out[1]
                perr    = np.sqrt(np.diag(pcov))
                e_R200  = perr[0]
                e_c200  = perr[1]
    ...
    def __init__(self,R,rho,err,z, cosmo,fitc = False):
        ...
        if fitc:
            ...
            try:
                ...
                NFW_out = curve_fit(NFW_profile,R,rho,sigma=err,absolute_sigma=True, bounds=([0.,0],[2.5,10]))
                pcov    = NFW_out[1]
                perr    = np.sqrt(np.diag(pcov))
                e_R200  = perr[0]
                e_c200  = perr[1]
